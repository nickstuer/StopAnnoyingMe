...
function SpellsUI:InitializeOptions()
	...
	subtitle:SetText("Select which spells to automatically remove. Make sure to reload the UI after making changes.")
	yPos = yPos - 25
   ...
   local headerBNetFavorite1 = scrollChild:CreateFontString(nil, "ARTWORK", "GameFontNormalLarge")
	...
	headerBNetFavorite1:SetText("Spell/Buff")
	...
	headerBNetFavorite2:SetText("Remove")
    yPos = yPos - 25


    for k, v in ipairs(SAMDB.SpellsToRemove) do

        local icon = "interface/icons/inv_misc_questionmark.blp"

        local spell = C_Spell.GetSpellInfo(SAMDB.SpellsToRemove[k]["spellID"])
        if spell then
            icon =  spell.iconID
        end
        ...
        option:SetText("|T" .. icon .. ":20:20:0:0|t " .. SAMDB.SpellsToRemove[k]["name"] .. " (" .. SAMDB.SpellsToRemove[k]["spellID"] .. ")")
        ...
        local _, newY = UIHelper.CreateCheckbox(
            ...
            300,
            yPos + 7,
            SAMDB.SpellsToRemove[k]["enabled"],
            function(self)
                SAMDB.SpellsToRemove[k]["enabled"] = self:GetChecked()
            end
        )
